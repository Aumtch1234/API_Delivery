...
exports.getDailySummary = async (req, res) => {
  try {
    ...
    const hourlySql = `
      WITH filtered_orders AS (
        SELECT (o.created_at AT TIME ZONE 'Asia/Bangkok') AS created_local, o.total_price
        FROM orders o
        WHERE DATE(o.created_at AT TIME ZONE 'Asia/Bangkok') = $1
          AND ($2::int IS NULL OR o.market_id = $2)
          AND (array_length($3::text[],1) IS NULL OR o.status = ANY($3::text[]))
...
exports.getMonthlySummary = async (req, res) => {
  ...
  SELECT
    (SELECT COALESCE(SUM(total_price),0)::numeric(12,2) FROM filtered_orders) AS total_monthly_revenue,
    (SELECT COUNT(*)::int FROM filtered_orders) AS total_monthly_orders,
    (
      SELECT json_agg(row_to_json(t) ORDER BY t.d)
      FROM (
        SELECT 
          c.d,
          COALESCE(COUNT(f.d), 0)::int AS orders,
          COALESCE(SUM(f.total_price), 0)::numeric(12,2) AS revenue
        FROM calendar c
    ...
    const sql = `
      ...
      monthly AS (
        SELECT
          ...
          COALESCE(COUNT(fo.order_id),0)::int AS orders,
          COALESCE(SUM(fo.total_price),0)::numeric(12,2) AS revenue
        FROM month_cal mc
      ...
      SELECT
        (SELECT COALESCE(SUM(total_price),0)::numeric(12,2) FROM filtered_orders) AS total_yearly_revenue,
        (SELECT COUNT(*)::int FROM filtered_orders) AS total_yearly_orders,
